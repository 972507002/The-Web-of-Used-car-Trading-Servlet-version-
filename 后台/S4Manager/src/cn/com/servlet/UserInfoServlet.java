...
public class UserInfoServlet extends HttpServlet{
	...
	protected void service(HttpServletRequest req, HttpServletResponse resp)
		...
		deleteYzm(session); //删除已有验证码
	        //登录操作
		 if(op.equals("login")){
			 if(session.getAttribute("userinfo")==null){	  //已登录验证
		...
		if(_userInfo!=null){//账户密码验证
			Date date=new Date();
			 DateFormat dateFormat=new SimpleDateFormat("HH");
			 
	   if(_userInfo.getU_admin().equals("管理员")){ //权限验证
		  ...
		  String loginmessage="没有权限";
			...
			String loginmessage=null;
			 if(userInfoService.getUserInfoByUnique(userInfo)==null){ //账户验证
				 DbUtil.closeAll();
				 loginmessage="该用户不存在";
				 String sRand= getYZM(session);
				    session.setAttribute("sRand", sRand);
