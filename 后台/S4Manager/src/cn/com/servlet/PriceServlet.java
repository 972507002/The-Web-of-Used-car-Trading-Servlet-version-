...
DefaultPieDataset dfp = new DefaultPieDataset();   //饼装数据集
private IPriceService iPriceService=new PriceServiceImpl();
	...
	else
	{
		...
		String chartkind=new String(req.getParameter("sel4").getBytes("ISO8859-1"),"UTF-8");//页面传递图片类型
		...
		if(year>0&month==0&day==0)
		{
			dataset = getyearDataSet();  
		        ...
		        String filepath =session.getServletContext().getRealPath("/yeji")+"/"+name+".jpeg";
		      
		        FileOutputStream out=new FileOutputStream(new File(filepath));
		        
		 ChartUtilities.writeChartAsJPEG(out, chart, 700, 700); //讲图片以jpeg格式保存
