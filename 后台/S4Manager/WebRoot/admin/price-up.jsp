...
<div id="main" class="wrap">
	...
	<div class="main">
		<h2>新增用户</h2>
		<div class="manage">
			<form action="/S4Manager/Model.action" method="post" onsubmit="return chesub('minAge','maxAge','万','cAge','chekAge')">
				<input type="hidden" name="op" value="tjupprice"/>
				<input type="hidden" name="pid" value="${price.p_id }"/>
				<table class="form">
				<tr><td>目前车价区间</td><td><input type="text"  value="${price.p_name }" disabled="disabled"/></td></tr>
				<tr><td>最小车价</td><td><input type="text" id="minAge" name="minAge" onblur="chekSubMit('minAge','maxAge','万','cAge','chekAge')" /></td></tr>
				<tr><td>最大车价</td><td><input type="text" id="maxAge" name="maxAge"  onblur="chekSubMit('minAge','maxAge','万','cAge','chekAge')"/><div id="chekAge" style="display: none;"></div></td></tr>
				<tr><td>热度</td><td colspan="4"><input type="text" id="count" value="${price.p_count }" name="count" onblur="chekCount()" /><div id="d2" >${mea }</div></td><td></td></tr>
				<tr><td>注</td><td>只输入最小车价则记录为X万以上，只输入最大车价则记录为X万以内,两者都输则记录为X-X万,热度高的会在前台优先展示,不输入则记为0</td></tr>	
					
					<tr>
						<td><input type="hidden" id="cAge" value="${price.p_name }" name="cAge"/></td>
