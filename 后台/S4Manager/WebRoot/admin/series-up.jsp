...
<div id="main" class="wrap">
	...
	<div class="main">
		...
		<form action="Model.action" method="post" onsubmit="return chekSub()">
			<table class="list">
				<tr><td>品牌：<input type="hidden" name="op" value="tjupser" /><input type="hidden" name="mid" value="${ model.m_id} "/></td><td><select name="sel1" id="sel1" onchange="chekexz()" >
				<option value="选择品牌">请选择品牌</option>
								  <c:if test="${!empty allbrand }">
                    <c:forEach var="allbrand" items="${allbrand }">
                    <c:if test="${model.b_id==allbrand.value.b_id }">
                                        <option selected="selected"  value=${allbrand.value.b_id }>${allbrand.value.b_name }</option>
				...
				</select><div id="spp" style="display: none;" ></div><input type="hidden" name="cpp" id="cpp" value="${model.b_id}" /></td></tr>
				<tr><td>车系：</td><td> <input type="text" id="name" name="name" value="${model.m_name}"  onblur="chek()" /><div id="dea" >${mea }</div></td></tr>
