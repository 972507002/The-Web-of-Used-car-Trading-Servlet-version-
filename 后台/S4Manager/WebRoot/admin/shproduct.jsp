...
{         
 ...
 function chekprice(id,id2,id3){
 
 var newprice=document.all(id);

 var price=document.all(id3);
 
 var div=document.all(id2);
 ...
 if(isNumber(newprice.value)){
   if(parseFloat(newprice.value)<parseFloat(price.value)){
     div.innerHTML="<font color='red'>旧车价高于新车价的车不予通过</font>";
     div.style.display="inline";
   }
   else{
   flag=true;
...
var url="<%=basePath%>MgCar.action?op=jjtg&cid="+a+"&uid="+b;
...
{
alert("操作成功")
window.location.href="<%=basePath%>MgCar.action?op=showwtg"
}
else
{
alert("删除失败")}
...
function yxtg(a,b,id,id2,id3)
{

if(chekprice(id,id2,id3))
{
var newprice= document.all(id).value;

var url="<%=basePath%>MgCar.action?op=yxtg&cid="+a+"&uid="+b+"&newprice="+newprice;
jQuery.post(url,null,callback)
function callback(data)
{
if(data==1)
{
alert("操作成功")
window.location.href="<%=basePath%>MgCar.action?op=showzs"
}
else
{
alert("删除失败")}
...
<div id="main" class="wrap">
	...
	<div class="main">
		...
		<input type="hidden" name="op" value="showshwhere"/>
		 ...
		 <select  name="sel2" id="sel2" onchange="getcx(this)" >
			...
			<table class="list">
				...
				<td>${carMap.u_id }</td>
					...
					<td class="thumb" ><a target="_blank" href="/S4/CarInfo.action?op=showshdea&cid=${carMap.c_id }&uid=${carMap.u_id}"><img src="/S4/${carMap.c_img}" /></a><a target="_blank" href="/S4/CarInfo.action?op=showshdea&cid=${carMap.c_id }&uid=${carMap.u_id}">${carMap.c_brand } ${carMap.c_series } ${carMap.c_id }${carMap.c_type }${carMap.c_model }&nbsp${carMap.c_price }万元</a></td>
					<td><input type="text" name="newprice" id="newprice${carMap.c_id }" onblur="chekprice('newprice${carMap.c_id }','d${carMap.c_id }','p${carMap.c_id }')"/><div id="d${carMap.c_id }" style="display: none"></div><input type="hidden" id="p${carMap.c_id}" value="${ carMap.c_price}"/></td><td>${carMap.c_state }</td>
					<td class="w1 c"> <input type="button" value="允许" onclick="yxtg(${carMap.c_id},${carMap.u_id },'newprice${carMap.c_id }','d${carMap.c_id }','p${carMap.c_id }')"></input><input type="button" value="拒绝" onclick="jjtg(${carMap.c_id},${carMap.u_id })"></input></td>
				</tr>
				</c:forEach>
				</c:if>
			 ...
			 <div class="pages" align="center"> <c:if test="${curPage > 1}">
					<a href="/S4Manager/MgCar.action?op=showshwhere&cpp=${pp }&ccx=${cx}&jumpPage=1">首页</a>&nbsp;&nbsp;&nbsp;
		 		<a href="/S4Manager/MgCar.action?op=showshwhere&cpp=${pp }&ccx=${cx}&jumpPage=${curPage - 1}&">上一页</a>
				</c:if> <c:if test="${curPage <= 1}">
			 		首页&nbsp;&nbsp;&nbsp;上一页
		 		</c:if> &nbsp;&nbsp;&nbsp; <c:if test="${curPage < maxPage}">
					<a href="/S4Manager/MgCar.action?op=showshwhere&cpp=${pp }&ccx=${cx}&jumpPage=${curPage + 1}">下一页</a>
		 			&nbsp;&nbsp;&nbsp;
		 			<a href="/S4Manager/MgCar.action?op=showshwhere&cpp=${pp }&ccx=${cx}&jumpPage=${maxPage}">尾页</a>
				</c:if> <c:if test="${curPage >= maxPage}">
					下一页
		 			&nbsp;&nbsp;&nbsp;尾页		 		
		 		</c:if>
