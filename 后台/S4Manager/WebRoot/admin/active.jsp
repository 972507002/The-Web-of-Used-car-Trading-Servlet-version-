...
{
alert("删除成功")
window.location.href="<%=basePath%>Trends.action?op=showactive"
}
else
{
alert("删除失败")}
		...
		<div style="width:750px; height:600px; overflow:scroll;">
			<table class="list">
				...
				<tr>
					...
					<td class="w1 c"><a href="Trends.action?op=upactive&tid=${trends.value.tr_id }">修改</a> <a onclick="udelete(${trends.value.tr_id})">删除</a></td>
				</tr>
			 ...
			 <div class="pages"> <c:if test="${curPage > 1}">
					<a href="Trends.action?op=showactive&jumpPage=1">首页</a>&nbsp;&nbsp;&nbsp;
		 		<a href="Trends.action?op=showactive&jumpPage=${curPage - 1}">上一页</a>
				</c:if> <c:if test="${curPage <= 1}">
			 		首页&nbsp;&nbsp;&nbsp;上一页
		 		</c:if> &nbsp;&nbsp;&nbsp; <c:if test="${curPage < maxPage}">
					<a href="Trends.action?op=showactive&jumpPage=${curPage + 1}">下一页</a>
		 			&nbsp;&nbsp;&nbsp;
		 			<a href="Trends.action?op=showactive&jumpPage=${maxPage}">尾页</a>
				</c:if> <c:if test="${curPage >= maxPage}">
					下一页
		 			&nbsp;&nbsp;&nbsp;尾页		 		
		 		</c:if>
