...
<div id="header" class="wrap">
	...
	<div class="navbar">
		<ul class="clearfix">
			...
			<li class="current"><a href="/S4Manager/Trends.action?op=showcom">留言</a></li>
	...
	<div class="main">
		...
		<div class="manage">
			<form action="/S4Manager/Trends.action" method="post" onsubmit="return cheksub('maxAge','chekAge','zm','chekZm')">
				<input type="hidden" name="op" value="tjupcom"/>
				<input type="hidden" name="cid" value="${ucomment.cid }"/>
								<input type="hidden" name="qtp" value="${ucomment.c_img }"/>
				
				<table class="form">
				
				<tr><td>评论标题</td><td><input type="text" id="maxAge" name="maxAge" value="${ucomment.c_bt }"  onblur="cheknull('maxAge','chekAge')"/><div id="chekAge" >${cmea }</div></td></tr>
				<tr><td>评论内容</td><td><textarea id="zm" disabled="disabled" name="zm"  onblur="cheknull('zm','chekZm')">${ucomment.c_text }</textarea><div id="chekZm" style="display: none;"></div></td></tr>
				
				<tr><td>评论头图片</td><td ><div align="center" style="height:200;border:5px solid red; " id="fa1"><img src="/S4Manager/${ucomment.c_img }" /></div><div style=" height:0px; overflow:hidden; position:absolute;" ><input type="file" id="filePath" name="filePath" onchange="fileupload('filePath','fa1','ttp')"/></div>
                
           <input type="button" onclick="document.getElementById('filePath').click();" value="上传" /></td></tr>
				
				<tr><td >评论类型</td><td>
				<select id="sel1" name="sel1" onchange="chekxz()">
				<option value="0">请选择</option>
				<c:if test="${ucomment.c_admin=='首页展示' }">
				<option selected="selected"  value="首页展示">首页展示</option>
				<option value="普通">普通</option>
				</c:if>
				<c:if test="${ucomment.c_admin=='普通' }">
				<option value="首页展示">首页展示</option>
				<option selected="selected" value="普通">普通</option>
				</c:if>
				</select>
				<div id="d2" style="display: none"></div></td></tr>
					<tr>
