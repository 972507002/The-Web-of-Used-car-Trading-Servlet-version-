...
function chek (str,str2){
 ...
 if(isNumber(text.value)){
   if(text.value<100000){
   flag=true;
   }
   else{
   div.innerHTML="<font color='red'>最多允许1000000</font>";
...
function chekSubMit(str,str1,str2,str3,str4){
 ...
 var where=document.all(str3);
  if(chek (str,str4)&&chek(str1,str4)){
     ...
     if(parseInt(text1.value)>parseInt(text.value)){
...
<div id="main" class="wrap">
	...
	<div class="main">
		<h2>新增用户</h2>
		<div class="manage">
			<form action="/S4Manager/Model.action" method="post" onsubmit=" return chesub('minAge','maxAge','万公里','cAge','chekAge')">
				<input type="hidden" name="op" value="tjupdis"/>
				<input type="hidden" name="did" value="${distance.d_id }"/>
				<table class="form">
				<tr><td>目前距离区间</td><td><input type="text" value="${distance.d_name }" disabled="disabled"/></td></tr>
				<tr><td>最小距离</td><td><input type="text" id="minAge" name="minAge" onblur="chekSubMit('minAge','maxAge','万公里','cAge','chekAge')" /></td></tr>
				<tr><td>最大距离</td><td><input type="text" id="maxAge" name="maxAge"  onblur="chekSubMit('minAge','maxAge','万公里','cAge','chekAge')"/><div id="chekAge" >${mea }</div></td></tr>
				<tr><td>热度</td><td colspan="4"><input type="text" value="${distance.d_count }" id="count" name="count" onblur="chekCount()" /><div id="d2" style="display: none;"></div></td><td></td></tr>
				<tr><td>注</td><td>只输入最小距离则记录为X万公里以上，只输入最大距离则记录为X万公里以内,两者都输则记录为X-X万公里,热度高的会在前台优先展示,不输入则记为0</td></tr>	
					
					<tr>
						<td><input type="hidden" id="cAge" value="${distance.d_name }" name="cAge"/></td>
