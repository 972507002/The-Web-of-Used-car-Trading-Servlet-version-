	...
	<div class="main">
		...
		<div class="manage">
			<form action="/S4Manager/Trends.action" method="post" onsubmit="return cheksub('maxAge','chekAge','zm','chekZm')">
				...
				<tr><td>动态消息标题</td><td><input type="text" id="maxAge" name="maxAge" value="${utrends.tr_title }"  onblur="cheknull('maxAge','chekAge')"/><div id="chekAge" >${tmea }</div></td></tr>
				<tr><td>动态消息内容</td><td><textarea id="zm" name="zm"  onblur="cheknull('zm','chekZm')">${utrends.tr_text }</textarea><div id="chekZm" style="display: none;"></div></td></tr>
				<tr><td>动态消息类型</td><td colspan="4"><select onchange="chekxz()"  id="sel1" name="sel1">
				<option value="0">请选择</option>
				<c:if test="${utrends.tr_type=='新闻' }">
				<option selected="selected" value="新闻">新闻</option>
				<option value="活动">活动</option>
				<option value="承诺">承诺</option>
				<option value="指南">指南</option>
				</c:if>
				<c:if test="${utrends.tr_type=='活动' }">
				<option  value="新闻">新闻</option>
				<option selected="selected" value="活动">活动</option>
				<option value="承诺">承诺</option>
				<option value="指南">指南</option>
				</c:if>
				<c:if test="${utrends.tr_type=='指南' }">
				<option  value="新闻">新闻</option>
				<option value="活动">活动</option>
				<option value="承诺">承诺</option>
				<option selected="selected" value="指南">指南</option>
				</c:if>
				<c:if test="${utrends.tr_type=='承诺' }">
				<option  value="新闻">新闻</option>
				<option value="活动">活动</option>
				...
				<tr><td>动态消息头图片</td><td ><div align="center" style="height:200;border:5px solid red; " id="fa1"><img src="/S4Manager/${utrends.tr_img }" /></div><div style=" height:0px; overflow:hidden; position:absolute;" ><input type="file" id="filePath" name="filePath" onchange="fileupload('filePath','fa1','ttp')"/></div>
