...
var url="<%=basePath%>Mgord.action?op=tgzz&cid="+cid+"&cuid="+cuid+"&uid="+uid;
...
var url="<%=basePath%>Mgord.action?op=jjzz&cid="+cid+"&cuid="+cuid+"&uid="+uid;
jQuery.post(url,null,callback)
function callback(data){
if(data==1)
{
		...
		<!--<form method="get">
			...
			<table class="list">
				...
				<tr>
					...
					<c:if test="${sm.value.p_state=='暂停交易' }">
			<td class="w1 c"><input type="button" value="通过" onclick="pass(${sm.value.c_id},${sm.value.c_uid },${sm.value.u_id })"/><input type="button" value="拒绝" onclick="nopass(${sm.value.c_id},${sm.value.c_uid },${sm.value.u_id })"/></td>
					</c:if>
					<c:if test="${ sm.value.p_state=='交易完成'}">
					             <td class="w1 c">交易完成的单只有老板可以操作</td>
					</c:if>
					<c:if test="${sm.value.p_state!='暂停交易' and sm.value.p_state!='交易完成' }">
			 ...
			 <div class="pages" align="center"> <c:if test="${curPage > 1}">
					<a href="/S4Manager/Mgord.action?op=showallord&jumpPage=1">首页</a>&nbsp;&nbsp;&nbsp;
		 		<a href="/S4Manager/Mgord.action?op=showallord&jumpPage=${curPage - 1}&">上一页</a>
				</c:if> <c:if test="${curPage <= 1}">
			 		首页&nbsp;&nbsp;&nbsp;上一页
		 		</c:if> &nbsp;&nbsp;&nbsp; <c:if test="${curPage < maxPage}">
					<a href="/S4Manager/Mgord.action?op=showallord&jumpPage=${curPage + 1}">下一页</a>
		 			&nbsp;&nbsp;&nbsp;
		 			<a href="/S4Manager/Mgord.action?op=showallord&jumpPage=${maxPage}">尾页</a>
				</c:if> <c:if test="${curPage >= maxPage}">
					下一页
		 			&nbsp;&nbsp;&nbsp;尾页		 		
		 		</c:if>
