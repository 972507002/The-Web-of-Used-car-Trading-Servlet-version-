		...
		<!--<form method="get">
			...
			<table class="list">
				...
				<tr>
					...
					<c:if test="${car.value.c_id==sm.value.c_id }">
					<td class="w1 c">${car.value.c_price }万元</td>
					<td class="w1 c">${car.value.c_mcsj }</td>
					</c:if>
					</c:forEach>
					
					<td class="w1 c">交易完成的订单只有老板可以操作</td>
				</tr>
				</c:forEach>
				</c:if>
			 ...
			 <div class="pages" align="center"> <c:if test="${curPage > 1}">
					<a href="/S4Manager/Mgord.action?op=showjywc&jumpPage=1">首页</a>&nbsp;&nbsp;&nbsp;
		 		<a href="/S4Manager/Mgord.action?op=showjywc&jumpPage=${curPage - 1}&">上一页</a>
				</c:if> <c:if test="${curPage <= 1}">
			 		首页&nbsp;&nbsp;&nbsp;上一页
		 		</c:if> &nbsp;&nbsp;&nbsp; <c:if test="${curPage < maxPage}">
					<a href="/S4Manager/Mgord.action?op=showjywc&jumpPage=${curPage + 1}">下一页</a>
		 			&nbsp;&nbsp;&nbsp;
		 			<a href="/S4Manager/Mgord.action?op=showjywc&jumpPage=${maxPage}">尾页</a>
				</c:if> <c:if test="${curPage >= maxPage}">
					下一页
		 			&nbsp;&nbsp;&nbsp;尾页		 		
		 		</c:if>
