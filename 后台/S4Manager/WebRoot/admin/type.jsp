...
var url="<%=basePath%>Model.action?op=deltype&tid="+a;
...
{
alert("删除成功")
window.location.href="<%=basePath%>Model.action?op=showtype"
}
else
{
alert("删除失败")}
	...
	<div class="main">
		...
		<div class="manage">
			<table class="list">
				...
				<tr>
					<td class="first w4 c">${brandMap.value.t_id}</td>
					<td><img src="${brandMap.value.t_class }"/>${brandMap.value.t_name }</td>
					<td>${brandMap.value.t_count }</td>
					<td class="w1 c"><a href="Model.action?op=uptype&tid=${brandMap.value.t_id }">修改</a> <a onclick="udelete(${brandMap.value.t_id})">删除</a></td>
				</tr>
				</c:forEach>
				</c:if>
