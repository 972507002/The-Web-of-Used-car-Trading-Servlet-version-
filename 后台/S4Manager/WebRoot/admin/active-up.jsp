...
管理员<a href="/S4Manager/MgUsIn.action?op=showdeauser&uid=${userinfo.u_id }">${userinfo.u_name }</a>您好，今天是<input type="text" name='time' id="time" style="border: 0px;background:#fc7e31" readonly size='16' />，欢迎回到管理后台。	
	</div>
</div>
	...
	<div class="main">
		...
		<div class="manage">
			<form action="/S4Manager/Trends.action" method="post" onsubmit="return cheksub('maxAge','chekAge','zm','chekZm')">
				...
				<input type="hidden" name="tid" value="${utrends.tr_id }"/>
								<input type="hidden" name="qtp" value="${utrends.tr_img }"/>
				
				<table class="form">
				
				<tr><td>活动标题</td><td><input type="text" id="maxAge" name="maxAge" value="${utrends.tr_title }"  onblur="cheknull('maxAge','chekAge')"/><div id="chekAge" >${tmea }</div></td></tr>
				<tr><td>活动内容</td><td><textarea id="zm" name="zm"  onblur="cheknull('zm','chekZm')">${utrends.tr_text }</textarea><div id="chekZm" style="display: none;"></div></td></tr>
				
				<tr><td>活动头图片</td><td ><div align="center" style="height:200;border:5px solid red; " id="fa1"><img src="/S4Manager/${utrends.tr_img }" /></div><div style=" height:0px; overflow:hidden; position:absolute;" ><input type="file" id="filePath" name="filePath" onchange="fileupload('filePath','fa1','ttp')"/></div>
