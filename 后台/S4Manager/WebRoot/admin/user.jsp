...
function udelete(a)
{
var url="<%=basePath%>MgUsIn.action?op=del&id="+a;
...
{
alert("删除成功")
window.location.href="<%=basePath%>Manager.action?op=showalluser"
}
else
{
alert("删除失败")}
...
<div id="header" class="wrap">
	...
	<div class="navbar">
		<ul class="clearfix">
			...
			<li class="current"><a href="/S4Manager/Manager.action?op=showalluser">用户</a></li>
		...
		<div style="width:750px; height:600px; overflow:scroll;">
			<table class="list"  >
				...
				<tr>
					...
					<td >${u.value.u_adr }</td>
					<td>${u.value.u_admin }</td>
					<td class="w1 c"><c:if test="${u.value.u_admin=='管理员' }">
					<c:if test="${u.value.u_id==userinfo.u_id }">
				<font color="red">自己</font>	<a href="MgUsIn.action?op=uptadeuser&uid=${u.value.u_id }">修改</a><input type="button" value="删除" onclick="udelete(${u.value.u_id })"></input>
					</c:if>
					<c:if test="${u.value.u_id!=userinfo.u_id }">
					无权操作其他管理员
					</c:if>
					</c:if>
					<c:if test="${u.value.u_admin=='普通用户' }">
					<a href="MgUsIn.action?op=uptadeuser&uid=${u.value.u_id }">修改</a> <input type="button" value="删除" onclick="udelete(${u.value.u_id })"></input>
					</c:if></td>
				</tr>
				</c:forEach>
			</table>
			 ...
			 <div class="pages" align="center"> <c:if test="${curPage > 1}">
					<a href="Manager.action?op=showalluser&jumpPage=1">首页</a>&nbsp;&nbsp;&nbsp;
		 		<a href="Manager.action?op=showalluser&jumpPage=${curPage - 1}&">上一页</a>
				</c:if> <c:if test="${curPage <= 1}">
			 		首页&nbsp;&nbsp;&nbsp;上一页
		 		</c:if> &nbsp;&nbsp;&nbsp; <c:if test="${curPage < maxPage}">
					<a href="Manager.action?op=showalluser&jumpPage=${curPage + 1}">下一页</a>
		 			&nbsp;&nbsp;&nbsp;
		 			<a href="Manager.action?op=showalluser&jumpPage=${maxPage}">尾页</a>
				</c:if> <c:if test="${curPage >= maxPage}">
					下一页
		 			&nbsp;&nbsp;&nbsp;尾页		 		
		 		</c:if>
