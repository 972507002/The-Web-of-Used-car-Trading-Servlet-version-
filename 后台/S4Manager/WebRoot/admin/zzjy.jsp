...
{
alert('操作成功')
window.location.href="Mgord.action?op=showzzjy"
}
else
{
alert('操作失败')}
		...
		<!--<form method="get">
			...
			<table class="list">
			 ...
			 <div class="pages" align="center"> <c:if test="${curPage > 1}">
					<a href="/S4Manager/Mgord.action?op=showzzjy&jumpPage=1">首页</a>&nbsp;&nbsp;&nbsp;
		 		<a href="/S4Manager/Mgord.action?op=showzzjy&jumpPage=${curPage - 1}&">上一页</a>
				</c:if> <c:if test="${curPage <= 1}">
			 		首页&nbsp;&nbsp;&nbsp;上一页
		 		</c:if> &nbsp;&nbsp;&nbsp; <c:if test="${curPage < maxPage}">
					<a href="/S4Manager/Mgord.action?op=showzzjy&jumpPage=${curPage + 1}">下一页</a>
		 			&nbsp;&nbsp;&nbsp;
		 			<a href="/S4Manager/Mgord.action?op=showzzjy&jumpPage=${maxPage}">尾页</a>
				</c:if> <c:if test="${curPage >= maxPage}">
					下一页
		 			&nbsp;&nbsp;&nbsp;尾页		 		
		 		</c:if>
