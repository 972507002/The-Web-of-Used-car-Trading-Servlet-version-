...
var url="<%=basePath%>Trends.action?op=deltrends&tid="+a;
...
{
alert("删除成功")
window.location.href="<%=basePath%>Trends.action?op=showalltrends"
}
else
{
alert("删除失败")}
	...
	<div class="main">
		...
		<div style="width:750px; height:600px; overflow:scroll;">
			<table class="list">
				...
				<tr>
					<td class="first w4 c">${trends.value.tr_id }</td>
					<td>${trends.value.tr_title}</td>
					<td><img  width="200" src="${trends.value.tr_img}"/></td>
					<td>${trends.value.tr_text}</td>
					<td>${trends.value.tr_type }</td>
					<td>${trends.value.tr_date}</td>
					<td class="w1 c"><a href="Trends.action?op=uptrends&tid=${trends.value.tr_id }">修改</a> <a onclick="udelete(${trends.value.tr_id})">删除</a></td>
				</tr>
			 ...
			 <div class="pages"> <c:if test="${curPage > 1}">
					<a href="Trends.action?op=showalltrends&jumpPage=1">首页</a>&nbsp;&nbsp;&nbsp;
		 		<a href="Trends.action?op=showalltrends&jumpPage=${curPage - 1}">上一页</a>
				</c:if> <c:if test="${curPage <= 1}">
			 		首页&nbsp;&nbsp;&nbsp;上一页
		 		</c:if> &nbsp;&nbsp;&nbsp; <c:if test="${curPage < maxPage}">
					<a href="Trends.action?op=showalltrends&jumpPage=${curPage + 1}">下一页</a>
		 			&nbsp;&nbsp;&nbsp;
		 			<a href="Trends.action?op=showalltrends&jumpPage=${maxPage}">尾页</a>
				</c:if> <c:if test="${curPage >= maxPage}">
					下一页
		 			&nbsp;&nbsp;&nbsp;尾页		 		
		 		</c:if>
