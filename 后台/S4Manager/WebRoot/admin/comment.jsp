...
var url="<%=basePath%>Trends.action?op=delcom&cid="+a;
...
{
alert("删除成功")
window.location.href="<%=basePath%>Trends.action?op=showcom"
}
else
{
alert("删除失败")}
	...
	<div class="main">
		...
		<div style="width:750px; height:600px; overflow:scroll;">
			<table class="list">
				...
				<tr>
					<td class="first w4 c">${trends.value.cid }</td>
					<td>${trends.value.c_bt}</td>
					<td><img  width="200" src="${trends.value.c_img}"/></td>
					<td>${trends.value.c_text}</td>
					<td>${trends.value.c_admin}</td>
					<td>${trends.value.c_date}</td>
					<td class="w1 c"><a href="Trends.action?op=upcom&cid=${trends.value.cid }">修改</a> <a onclick=" udelete(${trends.value.cid})">删除</a></td>
				</tr>
			 ...
			 <div class="pages"> <c:if test="${curPage > 1}">
					<a href="Trends.action?op=showcom&jumpPage=1">首页</a>&nbsp;&nbsp;&nbsp;
		 		<a href="Trends.action?op=showcom&jumpPage=${curPage - 1}">上一页</a>
				</c:if> <c:if test="${curPage <= 1}">
			 		首页&nbsp;&nbsp;&nbsp;上一页
		 		</c:if> &nbsp;&nbsp;&nbsp; <c:if test="${curPage < maxPage}">
					<a href="Trends.action?op=showcom&jumpPage=${curPage + 1}">下一页</a>
		 			&nbsp;&nbsp;&nbsp;
		 			<a href="Trends.action?op=showcom&jumpPage=${maxPage}">尾页</a>
				</c:if> <c:if test="${curPage >= maxPage}">
					下一页
		 			&nbsp;&nbsp;&nbsp;尾页		 		
		 		</c:if>
