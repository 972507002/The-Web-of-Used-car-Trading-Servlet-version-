...
<div id="main" class="wrap">
	...
	<div class="main">
		...
		<c:if test="${!empty szCar }">
                            ...
                            <c:if test="${szCar.value.p_state=='处理中' }">
                            <div class="hPages">操作: <input type="button" onclick="ycl(${szCar.value.p_id})" value="已处理"></input> <a onclick="del(${szCar.value.p_id})">删除</a>   </div></c:if>
                             <c:if test="${szCar.value.p_state=='已处理' }">
                             <div class="hPages">操作: <a onclick="del(${szCar.value.p_id})">删除</a>   </div></c:if>
		 		...
		 		<a href="/S4Manager/Manager.action?op=showsrdz&jumpPage=${curPage - 1}">上一页</a>
					...
					<a href="/S4Manager/Manager.action?op=showsrdz&jumpPage=${curPage + 1}">下一页</a>
		 			&nbsp;&nbsp;&nbsp;
		 			<a href="/S4Manager/Manager.action?op=showsrdz&jumpPage=${maxPage}">尾页</a>
				</c:if> <c:if test="${curPage >= maxPage}">
					下一页
		 			&nbsp;&nbsp;&nbsp;尾页		 		
		 		</c:if>
