...
{
alert("删除成功")
window.location.href="<%=basePath%>Manager.action?op=showyclsrdz"
}
else
{
alert("删除失败")}
	...
	<div class="main">
		...
		<c:if test="${!empty szCar }">
                            ...
                            <tr><td colspan="2" align="center">需求详述：</td><td colspan="5">${szCar.value.p_miaoshu }</td></tr>
                             ...
                             <div class="hPages">操作: <a onclick="del(${szCar.value.p_id})">删除</a>   </div>
		 		...
		 		<a href="/S4Manager/Manager.action?op=showsrdz&jumpPage=${curPage - 1}">上一页</a>
					...
					<a href="/S4Manager/Manager.action?op=showyclsrdz&jumpPage=${curPage + 1}">下一页</a>
		 			&nbsp;&nbsp;&nbsp;
		 			<a href="/S4Manager/Manager.action?op=showyclsrdz&jumpPage=${maxPage}">尾页</a>
				</c:if> <c:if test="${curPage >= maxPage}">
					下一页
		 			&nbsp;&nbsp;&nbsp;尾页		 		
		 		</c:if>
