...
<c:if test="${!empty userinfo }"> <a id="MemberMenuChange" href="MemberCenter.action?op=showmenber" target="_self">会员中心</a></c:if>
    ...
    ${userinfo.u_name } 
      ...
      </a>|<a  href="UserInfo.action?op=zhuxiao&url=CarInfo.action?op=showCompare&cid=${cid}&uid=${uid}" >注销</a>|&nbsp;&nbsp;&nbsp;&nbsp;<img src="images/tel.png"/>			
      </c:if>
      </span>
		</div>
	</div>
	<div class="head-search">
		<div class="wrap clearfix">
            ...
            <c:forEach var="showcar" items="${showcar }">
                        <a href="CarInfo.action?op=showlist&bname=${ showcar.value.c_brand}&cser=${showcar.value.c_series} " target="_blank" >${ showcar.value.c_brand}${showcar.value.c_series}</a>
...
<div id="about">
  <div class="mTags">
    <div class="wrap"> <a href="CarInfo.action?op=show">启乘名车广场</a>><a >车辆对比</a> </div>
  </div>
      ...
      <div class="carSoureList clearfix">
        <ul class="carSul clearfix">
          ...
          <c:forEach var="pareCarInfo" items="${pareCarInfo }">
            <li> <a href="CarInfo.action?op=showdetails&uid=${pareCarInfo.value.u_id }&cid=${pareCarInfo.value.c_id }" target="_blank">
            <div class="imgOut_3"> <img src="${pareCarInfo.value.c_img}"> </div>
            <h1>${pareCarInfo.value.c_brand }${pareCarInfo.value.c_series } ${pareCarInfo.value.c_releaseyear } 款 ${pareCarInfo.value.c_volume }L ${pareCarInfo.value.c_gearType } ${pareCarInfo.value.c_code } ${pareCarInfo.value.c_model }</h1>
            </a> 
            <!--<a href="javascript:;" class="delte" onclick="balance('1179')"></a>-->
            <p class="pre"> <span>一口价：</span> <b>${pareCarInfo.value.c_price }</b>万 </p>
          </li>
          </c:forEach>
          </c:if>
        </ul>
        <div class="carSutl clearfix">
          <ul>
              ...
              <div class="con" style="display:block;">
                <table>
                  <tr>
                    ...
                    <c:if test="${ !empty pareCarInfo}">
                    <c:forEach var="pareCarInfo" items="${pareCarInfo }">
                    <td class="std1"><span>${pareCarInfo.value.c_distance }万公里</span></td>
                    </c:forEach>
                    </c:if>
                  ...
                  <tr>
                    <th><span>初次上牌日期</span></th>
                     <c:if test="${ !empty pareCarInfo}">
                    <c:forEach var="pareCarInfo" items="${pareCarInfo }">
                    <td class="std1"><span> ${pareCarInfo.value.c_licencetime }</span></td>
                  ...
                  <tr>
                    ...
                    <c:forEach var="pareCarInfo" items="${pareCarInfo }">
                     ...
                     <td class="std1"><span>${pareCarInfo.value.c_volume }</span></td>
                    </c:forEach>
                    </c:if>
                  ...
                  <tr>
                    ...
                    <c:forEach var="pareCarInfo" items="${pareCarInfo }">
                     ...
                     <td class="std1"><span> ${pareCarInfo.value.c_type }</span></td>
                    </c:forEach>
                    </c:if>
            ...
            <li>
              <p class="carSutl_top"> <span class="fl">销售信息(登陆后可查看)</span> <span class="fr"></span> </p>
              <div class="con">
                <table>
                  <tr>
                    <th><span>指导价</span></th>
                  ...
                  <tr>
                    ...
                    <c:forEach var="pareCarInfo" items="${pareCarInfo }">
                     ...
                     <td class="std1"><span> ${pareCarInfo.value.c_price }（万）</span></td>
                    </c:forEach>
                    </c:if>
            ...
            <li>
              <p class="carSutl_top"> <span class="fl">配置信息(登陆后可查看)</span> <span class="fr"></span> </p>
              <div class="con">
                <table>
                  <tr>
                    <th><span>座位数</span></th>
                  ...
                  <tr>
                    ...
                    <c:forEach var="pareCarInfo" items="${pareCarInfo }">
                     <c:forEach var="pareHardwareConfig" items="${pareHardwareConfig }">
                    <c:if test="${pareCarInfo.key==pareHardwareConfig.key }">
                    <td class="std1"><span>${pareHardwareConfig.value.consoleLceScreen }</span></td>
                    </c:if>
                    </c:forEach>
                    </c:forEach>
                    </c:if>
    ...
    <div class="p-detail" >
      <div class="p-dl" id="login" style="display: none"  >
        <form  action="UserInfo.action" method="post" name="form" id="form" onsubmit="return chekLogin('username','userpwd','yzm','_yzm','d1','d2','d3')">
          ...
          <input type="hidden" name="url" value="CarInfo.action?op=showCompare&cid=${cid}&uid=${uid}">
