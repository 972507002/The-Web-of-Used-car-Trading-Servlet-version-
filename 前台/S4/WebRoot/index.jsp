...
function sendUpdateYzm(){
 var url = "<%=basePath%>UserInfo.action?op=updateyzm&nocahe="+new Date().getTime()+"";
  CreateXMLHttp();
...
function sendLogin(){
 ...
 var url = "<%=basePath%>UserInfo.action?op=needLogin&nocahe="+new Date().getTime()+"";
...
function sendReg(){
 var url = "<%=basePath%>UserInfo.action?op=needreg&nocahe="+new Date().getTime()+"";
  CreateXMLHttp();
  if(xmlhttp){
     xmlhttp.open("GET",url, true); 
...
function getSpp(){
  if (xmlhttp.readyState == 4 && xmlhttp.status == 200){ //完成请求正确返回
     ...
     for(var i=0;i<json.length;i++){
      sel2.options.add(new Option(json[i].m_name,json[i].m_name));
     }
...
if(cheksuub("sel1","sel5","cx","scx","ccx","srcx","srcx","scx","ccx","years","cxi","scxi","ccxi","srcxi","minage","maxage","年","ccxi","chekagee","minprice","maxprice","万","cprice","chekpricee")){
 ...
 var url = "<%=basePath%>MemberCenter.action?op=comment&sel1="+sel1+"&ccx="+ccx+"&ccxi="+ccxi+"&cprice="+cprice+"&buytime="+buytime+"&content="+content+"&nocahe="+new Date().getTime()+"";
...
strs=data.split(":");
 ...
 a.innerHTML=""+strs[0]+"<br/><a href='MemberCenter.action?op=showmenberSc'>"+strs[1]+"</a><br/><a href='javascript:closeDiv()'>关闭窗口</a>";
 ...
 function (){
  ...
  <div class="top">
    <div class="wrap clearfix"> <a href="#" class="logo left"><img src="images/logo.png"/></a>
      ...
      <c:if test="${!empty userinfo }"><a href="MemberCenter.action?op=needmaiche">我要卖车</a></c:if>
       <c:if test="${empty userinfo }"><a onclick="sendLogin()">我要卖车</a></c:if>
      <c:if test="${!empty userinfo }"><a href="CarInfo.action?op=showsrdz">私人订制</a></c:if>
     ...
     <c:if test="${!empty userinfo }"> <a id="MemberMenuChange" href="MemberCenter.action?op=showmenber" target="_self">会员中心</a></c:if>
      ...
      <c:if test="${!empty userinfo }"> <c:if test="${time<12 }">上午好！</c:if><c:if test="${time>=12 }">  下午好！ </c:if> <a href="MemberCenter.action?op=showmenber"  >
    ${userinfo.u_name } 
...
<div id="banner">
  ...
  <div class="searchBox">
   ...
   <img src="images/in1.png"/>
    <form class="clearfix" action="CarInfo.action" method="get" >
    <input type="hidden" name="op" value="showlist">
  ...
  <div class="wrap clearfix"> 
    ...
    <div class="item-brand c-item">
      ...
      <div class="brand-wrapper">
        ...
        <div class="bd clearfix"> 
         <c:if test="${!empty showbrand }">
        <c:forEach var="showbrand" items="${showbrand }">
         <a href="CarInfo.action?op=showlist&bname=${ showbrand.value.b_name}" class="brand-item" target="_blank"><img src="${showbrand.value.b_img }" alt="${showbrand.value.b_name }"><span>${showbrand.value.b_name }</span></a>
        ...
        <div class="brand-list clearfix">
          ...
          <c:forEach var="leftszm" items="${leftszm }">
            <div class="brand-item-l clearfix">
              ...
              <c:if test="${allbrand.value.b_szm==leftszm }">
              <a target="_blank" href="CarInfo.action?op=showlist&bname=${ allbrand.value.b_name}" class="brand-item">${allbrand.value.b_name }</a>
              </c:if>
              </c:forEach>
              </c:if>
              </div>
          ...
          <c:forEach var="rightszm" items="${rightszm }">
            <div class="brand-item-l clearfix">
              ...
              <c:if test="${!empty allbrand }">
              <c:forEach var="allbrand" items="${allbrand }">
              <c:if test="${allbrand.value.b_szm==rightszm }">
                            <a target="_blank" href="CarInfo.action?op=showlist&bname=${ allbrand.value.b_name}" class="brand-item">${allbrand.value.b_name }</a>
    ...
    <div id="choose_model" class="item-model c-item">
      ...
      <div class="model-wrapper">
        ...
        <div class="bd"> 
          ...
          <c:if test="${!empty showType }">
           <c:forEach var="showtype" items="${showType }">
            ...
            <a href="CarInfo.action?op=showlist&tname=${ showtype.value.t_name}" target="_blank"  class="model-item">
            <div class="${showtype.value.t_img}"></div>
          <span class="car-modal-name">${showtype.value.t_name }</span></a>
           </c:forEach>
           </c:if>
        ...
        <c:if test="${!empty showType }">
        <c:forEach var="showtype" items="${showType }">
                  <div class="models-line"><a class="models-item-m"  href="CarInfo.action?op=showlist&tname=${ showtype.value.t_name}" target="_blank" >${showtype.value.t_name }</a>
                  <c:if test="${!empty typecar}">
                  <c:forEach var="typecar" items="${ typecar}">
                  <c:if test="${typecar.value.c_type==showtype.value.t_name }">
                  <a target="_blank" href="CarInfo.action?op=showlist&bname=${typecar.value.c_brand }&cser=${typecar.value.c_series }" class="models-item">${typecar.value.c_brand }${typecar.value.c_series }</a>
                  </c:if>
      ...
      <div class="bd">
       <c:if test="${!empty priceMap }">
            <c:forEach var="priceMap" items="${priceMap }">
             <a href="CarInfo.action?op=showlist&price=${priceMap.value.p_name}" >${priceMap.value.p_name }</a>
            </c:forEach>
            </c:if>
      </div>
    </div>
  ...
  <div class="main-a mDiv" id="box1">
    <div class="wrap">
      ...
      <c:if test="${!empty active1 }">
              <div class="thPic left dInline pve tBox"> <a href="Trends.action?op=showteia&tr_id=${active1.tr_id }"  target="_blank"> <img src="${active1.tr_img }" class="imgVt" /> <span class="icon icon_hd"></span></a> </div>
        ...
        <div class="tBox right pve dInline thPicRight">
          <div class="thPic1 left dInline pve">
            ...
            <c:forEach var="showcar" items="${showcar }">
            <li class="pve"> <a href="CarInfo.action?op=showdetails&uid=${showcar.value.u_id }&cid=${showcar.value.c_id } " target="_blank"> <span class="icon icon_th"></span>
                <div class="car-pic"> <img src="${showcar.value.c_img }" /> </div>
                <div class="price"> <font>一口价</font> <span class="num nBlue">${showcar.value.c_price} </span> <font>&nbsp;&nbsp;万元</font> </div>
                <p>${showcar.value.c_brand }${showcar.value.c_series}  ${showcar.value.c_releaseyear}款   ${showcar.value.c_volume}T ${showcar.value.c_gearType} ${showcar.value.c_code}${showcar.value.c_model}</p>
                </a></li>
            </c:forEach>
            </c:if>
          ...
          <c:if test="${!empty active2 }">
                    <div class="pve right dInline hdPic"> <a href="Trends.action?op=showteia&tr_id=${active2.tr_id }"  target="_blank"> <img src="${active2.tr_img }" class="imgVt" /> <span class="icon icon_hd"></span></a> </div>
  ...
  <div class="mDiv" id="box2">
    <div class="wrap">
        ...
        <div class="tBox left pve dInline thPicRight">
          <div class="thPic1 left dInline pve">
            ...
            <c:forEach var="newcar" items="${newcar }">
              <li class="pve"> <a href="CarInfo.action?op=showdetails&uid=${newcar.value.u_id }&cid=${newcar.value.c_id }" target="_blank"> <span class="icon icon_npro"></span>
                <div class="car-pic"> <img src="${newcar.value.c_img }" /> </div>
                <div class="price"> <font>一口价</font> <span class="num nBlue">${newcar.value.c_price }</span> <font>&nbsp;&nbsp;万元</font> </div>
                <p>${newcar.value.c_brand }${newcar.value.c_series } ${newcar.value.c_releaseyear } 款 ${newcar.value.c_volume  }T ${newcar.value.c_gearType  } ${newcar.value.c_code  }${newcar.value.c_model  }</p>
                </a></li>
            </c:forEach>
            </c:if>
     ...
     <c:if test="${!empty active3 }">
                   <li class="pve liHd"> <a href="Trends.action?op=showteia&tr_id=${active3.tr_id }"  target="_blank"> <img src="${active3.tr_img }" class="imgVt" /> <span class="icon icon_hd"></span></a> </li>
     
      </c:if>
              
             <c:if test="${!empty fourthcar }">
            <c:forEach var="fourthcar" items="${fourthcar }">
              <li class="pve"> <a href="CarInfo.action?op=showdetails&uid=${fourthcar.value.u_id }&cid=${fourthcar.value.c_id }" target="_blank"> <span class="icon icon_npro"></span>
                <div class="car-pic"> <img src="${fourthcar.value.c_img }" /> </div>
                <div class="price"> <font>一口价</font> <span class="num nBlue">${fourthcar.value.c_price }</span> <font>&nbsp;&nbsp;万元</font> </div>
                <p>${fourthcar.value.c_brand }${fourthcar.value.c_series } ${fourthcar.value.c_releaseyear } 款 ${fourthcar.value.c_volume  }T ${fourthcar.value.c_gearType  } ${fourthcar.value.c_code  }${fourthcar.value.c_model  }</p>
                </a></li>
            </c:forEach>
            </c:if>
            </ul>
        ...
        <c:if test="${!empty active4 }">
                <div class="thPic right dInline pve tBox"> <a href="Trends.action?op=showteia&tr_id=${active4.tr_id }"  target="_blank"> <img src="${active4.tr_img }" class="imgVt" /> <span class="icon icon_hd"></span></a> </div>
        </c:if>
      </div>
    </div>
  </div>
  <div class="mDiv main-a">
    <div class="wrap">
      ...
      <div class="jpBox">
        ...
        <c:if test="${!empty showbrand }">
        <c:forEach var="showbrand" items="${showbrand }">
         <a> <img src="${showbrand.value.b_img }" width="50" height="38" /> <span>${showbrand.value.b_name }</span> <i></i></a>
        </c:forEach>
        </c:if>
        ...
        <div class="jpDl" style="display: block;">
            <ul class="clearfix">
             ...
             <c:if test="${!empty showcar }">
            <c:forEach var="showcar" items="${showcar }">
        <li class="clearfix" style="position: relative;">    <a href="CarInfo.action?op=showdetails&uid=${showcar.value.u_id }&cid=${showcar.value.c_id }" target="_blank">
                <div class="carImg left dInline"> <img src="${showcar.value.c_img}" width="266" /> </div>
                <div class="right carTxt right dInline pve">
                  <div class="c-txt">
                    <h3><a href="CarInfo.action?op=showdetails&uid=${showcar.value.u_id }&cid=${showcar.value.c_id }" target="_blank">${showcar.value.c_brand }${showcar.value.c_series}  ${showcar.value.c_releaseyear}款   ${showcar.value.c_volume}T ${showcar.value.c_gearType} ${showcar.value.c_code}${showcar.value.c_model}</a></h3>
                    <p>${showcar.value.c_licencetime } |  ${showcar.value.c_distance }万公里 | ${showcar.value.c_volume }T | ${showcar.value.c_emissionstandard }</p>
                    <div class="price">
                      <p>亿金一口价：</p>
                      <i>￥</i> <span class="num nBlue">${showcar.value.c_price} </span><font> 万元</font><font>省:￥ 18.77万</font> </div>
                  </div>
                </div>
                </a></li>
             ...
             <div class="proMore"> <a href="CarInfo.action?op=showall" target="_blank">查看更多>></a> </div> 
              </div>
        <c:forEach var="showbrand" items="${showbrand }">
         <div class="jpDl" >
            <ul class="clearfix">
            ...
            <c:if test="${brandcar.value.c_brand==showbrand.value.b_name }">
            
              <li class="clearfix" style="position: relative;"> 
            <a href="CarInfo.action?op=showdetails&uid=${brandcar.value.u_id }&cid=${brandcar.value.c_id }" target="_blank">
                <div class="carImg left dInline"> <img src="${brandcar.value.c_img }" width="266" /> </div>
                <div class="right carTxt right dInline pve">
                  <div class="c-txt">
                    <h3><a href="CarInfo.action?op=showdetails&uid=${brandcar.value.u_id }&cid=${brandcar.value.c_id }" target="_blank">${brandcar.value.c_brand }${brandcar.value.c_series } ${brandcar.value.c_releaseyear } 款 ${brandcar.value.c_volume  }T ${brandcar.value.c_gearType  } ${brandcar.value.c_code  } ${brandcar.value.c_model }</a></h3>
                    <p>${brandcar.value.c_licencetime  } | ${brandcar.value.c_distance }万公里 | ${brandcar.value.c_volume}T | ${brandcar.value.c_emissionstandard }</p>
                    <div class="price">
                      <p>亿金一口价：</p>
                      <i>￥</i> <span class="num nBlue">${brandcar.value.c_price  } </span><font> 万元</font> <font> 省：￥ 18.77 万</font> </div>
                  </div>
                </div>
                </a>
            </li>
             ...
             <div class="proMore"> <a href="CarInfo.action?op=showlist&bname=${ showbrand.value.b_name}" target="_blank">查看更多>></a> </div> 
              </div>
    ...
    <div class="wrap">
      ...
      <div class="bdBox clearfix">
        <div class="left dLeft dInline">
          <ul class="clearfix">
            <li><a href="CarInfo.action?op=jybd"  target="_blank"> <img src="images/55d2c89756ef2.jpg" /></a></li>
            <li><a href="CarInfo.action?op=jybd"  target="_blank"> <img src="images/55d2c8be18345.jpg" /></a></li>
            <li><a href="CarInfo.action?op=djbd"  target="_blank"> <img src="images/55d2c8d1f2f42.jpg" /></a></li>
            <li><a href="CarInfo.action?op=djbd"  target="_blank"> <img src="images/55d2c8e954f6f.jpg" /></a></li>
          </ul>
        </div>
...
input:-ms-input-placeholder, textarea:-ms-input-placeholder {color:#000;}
          ...
          <form  action="MemberCenter.action"  class="dForm" name="dForm" id="dForm"   method="post"  onsubmit="return cheksuub('sel1','sel5','cx','scx','ccx','srcx','srcx','scx','ccx','years','cxi','scxi','ccxi','srcxi','minage','maxage','年','ccxi','chekagee','minprice','maxprice','万','cprice','chekpricee');">
            <h2>私人订制</h2>
            <p>专业车辆顾问为您服务</p>
              ...
              <tr>
                <td><select name="years" id="years" onchange="chekcx('years','cxi','scxi','ccxi','srcxi')">
                    ...
                    <option value="不限">不限</option>
                      ...
                      <option value="${age.value.a_name }">${age.value.a_name }</option>
                      </c:forEach>
                      </c:if>
    ...
    <div class="wrap">
      ...
      <div class="in-news clearfix">
        <div class="new-left dInline left">
          ...
          <a href="Trends.action?op=showtei&tr_id=${new1.tr_id }" class='aImg'><img src="${new1.tr_img }"  width='262' height='170' /></a>
            </c:if>
              <ul>
              <c:if test="${!empty news1 }">
              <c:forEach var="news1" items="${news1 }">
              <li>·<a href="Trends.action?op=showtei&tr_id=${news1.tr_id }">${news1.tr_title }</a></li>
              </c:forEach>
              </c:if>
              </ul>
              <a style="float:right" href="Trends.action?op=shownewslist" target="_blank">更多>></a> </div>
            <div class="new-dl right dInline">
             <c:if test="${!empty new6}">
          <a href="Trends.action?op=showtei&tr_id=${new6.tr_id }" class='aImg'><img src="${new6.tr_img }"  width='262' height='170' /></a>
            </c:if>
              <ul>
            ...
            <div class="bd">
              ...
              <c:if test="${!empty com1 }">
                ...
                <c:forEach var="com1" items="${com1 }">
                  <div class="bd-dl"> <a href="/S4/Trends.action?op=showcomd&cid=${com1.value.cid }" class="aImg"> <img src="${com1.value.c_img }" width="242" height="170" /> </a>
                    <h4>${com1.value.c_bt }</h4>
                    <p>${com1.value.c_text }</p>
                    <div align="right"> <a href="Trends.action?op=showcomd&cid=${com1.value.cid}" class="cMore">查看详情</a> </div>
                  </div>
                  </c:forEach>
                ...
                <c:forEach var="com2" items="${com2 }">
                  <div class="bd-dl"> <a href="Trends.action?op=showcomd&cid=${com2.value.cid }" class="aImg"> <img src="${com2.value.c_img }" width="242" height="170" /> </a>
                    <h4>${com2.value.c_bt }</h4>
                    <p>${com2.value.c_text }</p>
                    <div align="right"> <a href="Trends.action?op=showcomd&cid=${com2.value.cid}" class="cMore">查看详情</a> </div>
                  </div>
                  </c:forEach>
                ...
                <c:forEach var="com3" items="${com3 }">
                  <div class="bd-dl"> <a href="Trends.action?op=showcomd&cid=${com3.value.cid }" class="aImg"> <img src="${com3.value.c_img }" width="242" height="170" /> </a>
                    <h4>${com3.value.c_bt }</h4>
                    <p>${com3.value.c_text }</p>
                    <div align="right"> <a href="Trends.action?op=showcomd&cid=${com3.value.cid}" class="cMore">查看详情</a> </div>
                  </div>
                  </c:forEach>
    ...
    <div class="p-detail" >
      <div class="p-dl" id="login" style="display: none"  >
        <form  action="UserInfo.action" method="post" name="form" id="form" onsubmit="return chekLogin('username','userpwd','yzm','_yzm','d1','d2','d3')">
          ...
          <input type="hidden" name="url" value="CarInfo.action?op=show">
          ...
          <div class="login-check">
            <input type="checkbox" name="checkbox" value="true" style=" width:auto;" />
            <label>记住我</label>
      ...
      <div class="p-dl" id="uureg">
         <form class="registForm" action="UserInfo.action" onsubmit="return chekReg('regname','regpwd','regyzm','_regyzm','realname','d9','d6','d7','d5')"  method="post" name="reg" id="reg">
          <ul class="login-items">
            <li class="clearfix">
