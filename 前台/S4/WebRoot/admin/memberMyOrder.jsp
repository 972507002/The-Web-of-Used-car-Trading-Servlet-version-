            ...
            <p>
				...
				<div class="mr-detail">
					<div class="mr-tab clearfix">
						<a class="on" href="MemberCenter.action?op=showmenberOrder">我的订单</a>
					...
					<div class="me-box">
					 <div class="me-dl"  style="display:block;">
							...
							</p><div class="price clearfix" style="margin:5px 0;"><div class="left dInline pNum">	<font>一口价</font><br/><span class="num nBlue">${szCar.value.c_price}</span><font> 万元</font>	状态：${sta }		 </div>			<span class="num1 left" style="height:43px;padding-top:10px;"><em>新车价：${szCar.value.newprice }万元</em><br/>				为您节省：${szCar.value.newprice-szCar.value.c_price }万元 							</span>		</div> 	<div class="cs_bt clearfix">			<a  href="MemberCenter.action?op=qxdd&uid=${szCar.value.u_id}&cid=${szCar.value.c_id}" class="cs-q">取消订单</a>	
										<a href="MemberCenter.action?op=qrsh&cid=${szCar.value.c_id}&uid=${szCar.value.u_id}">确认收货</a>		</div>	</div></li></ul><div class="hPages">    </div>
							</c:forEach>
							</c:if>
								</div>
								 <div class="pages"> <c:if test="${curPage > 1}">
					<a href="MemberCenter.action?op=showmenberOrder&jumpPage=1">首页</a>&nbsp;&nbsp;&nbsp;
		 		<a href="MemberCenter.action?op=showmenberOrder&jumpPage=${curPage - 1}">上一页</a>
				</c:if> <c:if test="${curPage <= 1}">
			 		首页&nbsp;&nbsp;&nbsp;上一页
		 		</c:if> &nbsp;&nbsp;&nbsp; <c:if test="${curPage < maxPage}">
					<a href="MemberCenter.action?op=showmenberOrder&jumpPage=${curPage + 1}">下一页</a>
		 			&nbsp;&nbsp;&nbsp;
		 			<a href="MemberCenter.action?op=showmenberOrder&jumpPage=${maxPage}">尾页</a>
				</c:if> <c:if test="${curPage >= maxPage}">
					下一页
		 			&nbsp;&nbsp;&nbsp;尾页		 		
		 		</c:if>
