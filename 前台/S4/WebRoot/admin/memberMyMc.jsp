...
function sendXj(c_id,c_uid,nop){
 ...
 var url = "<%=basePath%>MemberCenter.action?op="+nop+"&cid="+c_id+"&uid="+c_uid+"&nocahe="+new Date().getTime()+"";
  CreateXMLHttp();
  if(xmlhttp){
...
function getXj(){
  if (xmlhttp.readyState == 4 && xmlhttp.status == 200){ //完成请求正确返回
    ...
    showDiv(data);
    window.location.href="MemberCenter.action?op=showmenberXj";
  }
}
...
strs=data.split(":");
 ...
 a.innerHTML=""+strs[0]+"<br/><a href='MemberCenter.action?op=showmenberXj'>"+strs[1]+"</a><br/><a href='javascript:closeDiv()'>关闭窗口</a>";
            ...
            <p>
				...
				<div class="mr-detail">
					<div class="mr-tab clearfix">
						...
						<a class="on" href="MemberCenter.action?op=showmenberMc">出售中的车</a>
						<a href="MemberCenter.action?op=showmenberSh">审核中的车</a>
                        <a href="MemberCenter.action?op=showmenberSc">我收藏的车型</a>
		           ...
		           <div class="me-dl"  style="display:block;">
							...
							<span>状态：${szCar.value.c_state}</span>
							</p><div class="price clearfix" style="margin:5px 0;"><div class="left dInline pNum">	<font>一口价</font><br/><span class="num nBlue">${szCar.value.c_price}</span><font> 万元</font>			 </div>				 				</div>		<div class="cs_bt clearfix">						<a  onclick="sendXj(${szCar.value.c_id},${szCar.value.u_id },'needxj')">取消上架</a>		</div>	</div></li></ul><div class="hPages">    </div>
							</c:forEach>
							</c:if>
								</div>
								 <div class="pages"> <c:if test="${curPage > 1}">
					<a href="MemberCenter.action?op=showmenberMc&jumpPage=1">首页</a>&nbsp;&nbsp;&nbsp;
		 		<a href="MemberCenter.action?op=showmenberMc&jumpPage=${curPage - 1}">上一页</a>
				</c:if> <c:if test="${curPage <= 1}">
			 		首页&nbsp;&nbsp;&nbsp;上一页
		 		</c:if> &nbsp;&nbsp;&nbsp; <c:if test="${curPage < maxPage}">
					<a href="MemberCenter.action?op=showmenberMc&jumpPage=${curPage + 1}">下一页</a>
		 			&nbsp;&nbsp;&nbsp;
		 			<a href="MemberCenter.action?op=showmenberMc&jumpPage=${maxPage}">尾页</a>
				</c:if> <c:if test="${curPage >= maxPage}">
					下一页
		 			&nbsp;&nbsp;&nbsp;尾页		 		
		 		</c:if>
