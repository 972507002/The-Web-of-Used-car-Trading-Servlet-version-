      ...
      <c:if test="${time>12 }">
        ...
        <div class="mr-detail">
          <div class="mr-tab clearfix"> <a href="MemberCenter.action?op=showmenberYbcar">我购买的车</a> 
            ...
            <a class="on" href="MemberCenter.action?op=showmenberbjj">审核未通过的车</a>
         <a  href="MemberCenter.action?op=showmenberJyz">被订购的车</a>
              <a href="MemberCenter.action?op=showmenberymw">出售成功的车</a>
          </div>
          <div class="me-box me-box1">
           ...
           <div class="me-dl"  style="display:block;">
							...
							排放标准：${szCar.value.c_emissionstandard}</span>
								...
								<font>一口价</font><br/><span class="num nBlue">${szCar.value.c_price}</span><font> 万元</font>			 </div>			
								<span class="num1 left" style="height:43px;padding-top:10px;"><em>新车价：${szCar.value.newprice }万元</em><br/>				为您节省：${szCar.value.newprice-szCar.value.c_price }万元 						</span>		</div>		
								<div class="cs_bt clearfix">			<a  href="MemberCenter.action?op=tjbjj&uid=${szCar.value.u_id}&cid=${szCar.value.c_id}" class="cs-q">申请审核</a>	
										<a href="MemberCenter.action?op=upmc&cid=${szCar.value.c_id}">修改信息</a>		</div>	</div></li></ul><div class="hPages">    </div>
										
							</c:forEach>
							</c:if>
								</div>
								 <div class="pages"> <c:if test="${curPage > 1}">
					<a href="MemberCenter.action?op=showmenberbjj&jumpPage=1">首页</a>&nbsp;&nbsp;&nbsp;
		 		<a href="MemberCenter.action?op=showmenberbjj&jumpPage=${curPage - 1}">上一页</a>
				</c:if> <c:if test="${curPage <= 1}">
			 		首页&nbsp;&nbsp;&nbsp;上一页
		 		</c:if> &nbsp;&nbsp;&nbsp; <c:if test="${curPage < maxPage}">
					<a href="MemberCenter.action?op=showmenberbjj&jumpPage=${curPage + 1}">下一页</a>
		 			&nbsp;&nbsp;&nbsp;
		 			<a href="MemberCenter.action?op=showmenberbjj&jumpPage=${maxPage}">尾页</a>
				</c:if> <c:if test="${curPage >= maxPage}">
					下一页
		 			&nbsp;&nbsp;&nbsp;尾页		 		
		 		</c:if>
