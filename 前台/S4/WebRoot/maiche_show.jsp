...
function sendSc(c_id,c_uid){
 var url = "<%=basePath%>MemberCenter.action?op=needsc&c_id="+c_id+"&c_uid="+c_uid+"&nocahe="+new Date().getTime()+"";
  CreateXMLHttp();
  if(xmlhttp){
     xmlhttp.open("GET",url, true); 
...
<c:if test="${!empty userinfo }"> <a id="MemberMenuChange" href="MemberCenter.action?op=showmenber" target="_self">会员中心</a></c:if>
    ...
    ${userinfo.u_name } 
      ...
      </a>|<a  href="UserInfo.action?op=zhuxiao&url=CarInfo.action?op=showdetails&cid=${cid}&uid=${uid}" >注销</a>|&nbsp;&nbsp;&nbsp;&nbsp;<img src="images/tel.png"/>			
      </c:if>
      </span> </div>
  </div>
...
<div id="about">
  ...
  </c:if></a> <span class="right">对这辆车不满意，我要 <c:if test="${!empty userinfo }"><a href="CarInfo.action?op=showsrdz">私人订制</a></c:if>
...
<div class="img_hd">
  ...
  <a class="bottom_a next_a" style="opacity: 0.7; "></a> 
    ...
    <div class="buyTxt right dInline">
      <h1>${detailsMap.value.c_brand } ${detailsMap.value.c_series }${detailsMap.value.c_releaseyear } 款 ${detailsMap.value.c_volume }T ${detailsMap.value.c_gearType } ${detailsMap.value.c_code } ${detailsMap.value.c_model }</h1>
      <div class="byDl clearfix">
        <div class="price" style="width:235px;">
          <p>亿金
            一口                    价：</p>
          <span class="num nBlue"> ${detailsMap.value.c_price } </span> <font>万元</font>
     ...
     新车价:${detailsMap.value.newprice }万<br>
      为您节省:${detailsMap.value.newprice-detailsMap.value.c_price }万
      ...
      <div class="byDl clearfix byDl_tab"> <span class="left bItem">
        <div> 上牌时间
          <p>${detailsMap.value.c_licencetime }</p>
        </div>
        </span> <span class="left bItem">
        <div> 行驶里程
          <p>${detailsMap.value.c_distance }万公里</p>
        </div>
        </span> <span class="left bItem">
        <div> 排量
          <p>${detailsMap.value.c_volume }T</p>
        </div>
        </span> <span class="left bItem">
        <div> 排放标准
          <p>${detailsMap.value.c_emissionstandard }</p>
        </div>
        </span> </div>
      <div class="byBtn clearfix"> <c:if test="${empty userinfo }"><a onclick="sendLogin()" >立即抢定</a> </c:if><c:if test="${!empty userinfo }"><a  onclick="sendDg(${detailsMap.value.c_id },${detailsMap.value.u_id },'needdg')" >立即抢定</a> </c:if><a href="CarInfo.action?op=showCompare&uid=${detailsMap.value.u_id }&cid=${detailsMap.value.c_id }" class="balance_2">加入对比</a>
        <div class="jiathis_style_32x32"> <c:if test="${empty userinfo }"><a onclick="sendLogin()"  class="jiathis " style="text-decoration:none; text-align:center;display:inline-block;height:35px;  margin-right:12px">收藏</a> </c:if> <c:if test="${!empty userinfo }"><a onclick="sendSc(${detailsMap.value.c_id },${detailsMap.value.u_id })"  class="jiathis " style="text-decoration:none; text-align:center;display:inline-block;height:35px;  margin-right:12px">收藏</a> </c:if></div>
        <!--<a href="#">置换</a>--> 
    ...
    <div class="buyDetail">
      ...
      <c:forEach var="detailsMap" items="${detailsMap }">
        <div id="cBox1" class="c_box">
          <h2>${detailsMap.value.c_brand } ${detailsMap.value.c_series }${detailsMap.value.c_releaseyear } 款 ${detailsMap.value.c_volume }T ${detailsMap.value.c_gearType } ${detailsMap.value.c_code } ${detailsMap.value.c_model }</h2>
          <div class="cbox-a">
            <div class="ca-table">
              <table>
                <tr>
                  <td class="ca-td1">上牌时间</td>
                  <td>${detailsMap.value.c_licencetime }</td>
                  <td class="ca-td1">年检到期时间</td>
                  <td>${basicInfo.aidd }</td>
                  <td class="ca-td1">强险到期时间</td>
                  <td>${basicInfo.srdt }</td>
                  <td class="ca-td1">车辆编号</td>
                  <td>${basicInfo.c_id }</td>
                </tr>
                <tr>
                  <td class="ca-td1">车身颜色</td>
                  <td>${basicInfo.bodyColor }</td>
                  <td class="ca-td1">车体形式</td>
                  <td>${detailsMap.value.c_type }</td>
                  <td class="ca-td1">变速箱</td>
                  <td>${hardwareConfig.cvt }</td>
                  <td class="ca-td1">排量</td>
                  <td>${detailsMap.value.c_volume }T</td>
                </tr>
                <tr>
                ...
                <div class="jb_infor left">
                  <table>
                    <tr>
                      ...
                      <td>${detailsMap.value.c_emissionstandard }</td>
                    </tr>
                    <tr>
                ...
                <div class="jb_infor right">
                  <table>
                    <tr>
                      ...
                      <td>${detailsMap.value.c_distance }万公里</td>
                    </tr>
                    <tr>
                      <td class="ca-td1">车源状态</td>
                      <td class="ca-td1">${detailsMap.value.c_state }</td>
                    </tr>
                  </table>
                </div>
                <div class="jb_infor left">
                ...
                <div class="jb_infor left">
                  <table>
                    ...
                    <tr>
                      ...
                      <td class="ca-td1">${hardwareConfig.consoleLceScreen }</td>
                    </tr>
                    <tr>
                ...
                <div class="jb_infor right">
                  <table>
                    ...
                    <tr>
                      ...
                      <td class="ca-td1">${detailsMap.value.c_volume }T</td>
                    </tr>
                    <tr>
              ...
              <div class="clearfix">
                <div class="jb_infor left">
                  <table>
                    <tr>
                      <td width="270">销售价</td>
                      <td>${detailsMap.value.c_price }（万）</td>
                    </tr>
                    <tr>
            ...
            <div class="ca-dl"> <span class="caItem on">详细描述</span>
              <div class="clearfix">
                <table>
                  <tr>
                    <td colspan="8"><p>${detailsMap.value.c_czzx }</p></td>
                  </tr>
                </table>
              </div>
            </div>
  ...
  <div class="wrap">
    <div class="y_like">
      ...
      <c:forEach var="bcar" items="${bcar }">
       <li style="width:250px; height:333px;"> <a href="CarInfo.action?op=showdetails&uid=${bcar.value.u_id }&cid=${bcar.value.c_id }" style="width:auto; padding:40px 20px;"> <img src="${bcar.value.c_img}" height="120" />
          <h3>${bcar.value.c_brand}${bcar.value.c_series}${bcar.value.c_releaseyear}款${bcar.value.c_volume}T ${bcar.value.c_gearType}${bcar.value.c_code}${bcar.value.c_model}</h3>
          <div class="price"> 寄售
            价 <span class="num nBlue">${bcar.value.c_price}</span> 万 <font> 省：￥ ${bcar.value.newprice-bcar.value.c_price } 万</font> </div>
          <p>${bcar.value.c_licencetime} | ${bcar.value.c_distance}万公里 |${bcar.value.c_volume}T | ${bcar.value.c_emissionstandard}</p>
          </a> </li>
      </c:forEach>
      </c:if>
       <c:if test="${empty bcar }">
...
<div id="footer">
  ...
  <div class="foot-b"> Copyright © 2015 qcauto All Rights Reserved 河南亿金有限公司 版权所有 豫ICP备13013744号-1<br/>
    地址：郑州市金水区花园北路与天伦路交汇处东50米路南 服务热线：400-033-7777<br/>
    </div>
</div>
<!--右侧内容的开始-->
    ...
    <div class="p-detail" >
      <div class="p-dl" id="login" style="display: none"  >
        <form  action="UserInfo.action" method="post" name="form" id="form" onsubmit="return chekLogin('username','userpwd','yzm','_yzm','d1','d2','d3')">
          ...
          <input type="hidden" name="url" value="CarInfo.action?op=showdetails&cid=${cid}&uid=${uid}">
