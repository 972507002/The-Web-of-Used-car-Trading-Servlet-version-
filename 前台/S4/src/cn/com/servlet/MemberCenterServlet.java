       ...
       req.getRequestDispatcher("admin/memberMyzzmr.jsp").forward(req, resp);
		...
		String c_id=req.getParameter("c_id");
			...
			if(op.equals("fukuan")){
				...
				if(carInfoService.getCarByWhere(carInfo1).get(carInfo1.getC_id()).getC_state().equals("在售")){
				perSonCar.setU_id(userInfo.getU_id());
	...
	BasicInfo basicInfo=new BasicInfo();
		...
		req.getRequestDispatcher("admin/upcarinfo.jsp").forward(req, resp);
			...
			if(op.equals("liuyan")){
				...
				comment.setC_text(new String(req.getParameter("text").getBytes("iso8859-1"),"UTF-8"));
				DateFormat dateFormat=new SimpleDateFormat("yyyy/MM/dd HH:mm:ss");
			    Date date=new Date();
			    comment.setC_date(dateFormat.format(date));
